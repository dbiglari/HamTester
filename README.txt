...
Building the HamTester jar using the build.xml file requires ant. To build the HamTester jar using the build.xml file, from the command line run:
...
To build the HamTester binary in Windows, from the command line run:

makeBinary.bat

This will produce a Windows binary called "HamTester.exe" in the dist folder. 

To build the HamTester binary in Linux, from the command line run:
